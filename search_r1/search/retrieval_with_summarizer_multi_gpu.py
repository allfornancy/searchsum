...
检索服务使用GPU 0-3，Summarizer调用GPU 4上的API
"""
...
class DenseRetriever:
    def __init__(self, config):
        ...
        # 使用多GPU加载FAISS索引
        if config.faiss_gpu:
            print("Loading FAISS index to multiple GPUs...")
            co = faiss.GpuMultipleClonerOptions()
            co.useFloat16 = True
            co.shard = True  # 重要：分片到多个GPU
            self.index = faiss.index_cpu_to_all_gpus(self.index, co=co)
            print("FAISS index loaded to all available GPUs")
...
def retrieve_endpoint(request: QueryRequest):
    ...
    for i, single_result in enumerate(results):
        ...
        # 如果需要摘要
        if request.use_summarizer:
            user_question = request.user_questions[i] if request.user_questions and i < len(request.user_questions) else request.queries[i]
...
def health_check():
    ...
    return {
        ...
        "summarizer_url": "http://localhost:5000",
        "gpu_used": "0,1,2,3 for retrieval, 4 for summarizer"
    }

if __name__ == "__main__":
    print("Starting enhanced retrieval server...")
    print("- Retrieval: GPU 0-3 (FAISS multi-GPU)")
    print("- Summarizer: GPU 4 (API on port 5000)")
